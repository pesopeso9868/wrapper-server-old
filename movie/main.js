...
module.exports = {
	...
	async meta(movieId) {
		...
		// Larger buffer for description tag
		const buffer = Buffer.alloc(512);
		fs.readSync(fd, buffer, 0, 512, 0);
		//There's an easier way to do this. -Roy Rooster
		const bufstr = buffer.toString()
		//const begTitle = buffer.indexOf('<title>') + 16;
		//const endTitle = buffer.indexOf(']]></title>');
		titlematch = bufstr.match(/<title><!\[CDATA\[(.*?)\]\]/)
		descmatch = bufstr.match(/<desc><!\[CDATA\[(.*?)\]\]/)
		durmatch = bufstr.match(/duration="([\d.,+-]*)"/)
		var title = ""
		var desc = ""
		var duration = 0;
		if(titlematch){
		...
		if(durmatch){
			duration = Number.parseFloat(durmatch[1]);
		}
